...
ext.grid.MARGINX = 3
ext.grid.MARGINY = 3
ext.grid.GRIDWIDTH = 10
ext.grid.GRIDHEIGHT = 10
...
function ext.grid.maximize_window()
  local win = window.focusedwindow()
  local rect = ext.grid.get(win)
  local f = {x = 0, y = 0, w = ext.grid.GRIDWIDTH, h = ext.grid.GRIDHEIGHT}
  if (rect.w == ext.grid.GRIDWIDTH and rect.h == ext.grid.GRIDHEIGHT) then
    f = {x = 3, y = 2, w = 5, h = 6}
  end
  ext.grid.set(win, f, win:screen())
end
...
function ext.grid.resizewindow_shortter()
